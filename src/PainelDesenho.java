...
/**
 * Cria desenhos de acordo com o tipo e eventos do mouse e com os botões selecionados
 *
...
public class PainelDesenho extends JPanel implements MouseListener, MouseMotionListener {
    ...
    Color corAtual = Color.BLACK, cor1 = Color.BLACK, cor2 = Color.BLACK;
    // Cor atual do primitivo
    ...
    ListaLigada listaSave = new ListaLigada();
    ...
    public Color getCor1(){ return this.cor1; }
    public Color getCor2(){ return this.cor2; }
    public void setCor1(Color cor1) {System.out.println(cor1); this.cor1 = cor1;}
    public void setCor2(Color cor2) {this.cor2 = cor2;}
    ...
    public void mousePressed(MouseEvent e) {
        ...
        } else if (tipo == TipoPrimitivo.FLOR){
            if (primeiraVez == true) {
                x1 = (int)e.getX();
                y1 = (int)e.getY();
                primeiraVez = false;
    ...
    public void desenharPrimitivos(Graphics g){
        ...
        if (tipo == TipoPrimitivo.PONTO){
            FiguraPontos.desenharPonto(g, (int)x, (int)y, "", getEsp(), getCorAtual());
            ...
            listaSave.adicionarPontoLista("PONTO", x/950, y/600, getEsp(), getCorAtual());
            //AAAAAAAAAAAAAAAAAAAAAAAAAAA
        }

        //Caso a opção selecionada seja a reta
        if (tipo == TipoPrimitivo.RETA){
            FiguraRetas.desenharReta(g, (int)x1, (int)y1, (int)x2, (int)y2, "", getEsp(), getCorAtual());
            ...
            //Registra um novo elemento a lista ligada
            listaSave.adicionarRetaLista("RETA", x1/950, y1/600, x2/950, y2/600, getEsp(), getCorAtual());
        }

        //Caso a opção selecionada seja o retangulo
        if (tipo == TipoPrimitivo.RETANGULO){
            FiguraRetangulo.desenharRetangulo(g, (int)x1, (int)y1, (int)x2, (int)y2, "", getEsp(), getCorAtual());
            ...
            //Registra um novo elemento a lista ligada
            listaSave.adicionarRetaLista("RETANGULO", x1/950, y1/600, x2/950, y2/600, getEsp(), getCorAtual());
        }

        //Caso a opção selecionada seja o triangulo
        if (tipo == TipoPrimitivo.TRIANGULO){
            FiguraTriangulo.desenharTriangulo(g, (int)x1, (int)y1, (int)x2, (int)y2, (int)x3, (int)y3, "", getEsp(), getCorAtual());
            ...
            //Registra um novo elemento a lista ligada
            listaSave.adicionarTrianguloLista("TRIANGULO", x1/950, y1/600, x2/950, y2/600, x3/950, y3/600, getEsp(), getCorAtual());
        }

        //Caso a opção selecionada seja o circulo
        if (tipo == TipoPrimitivo.CIRCULO){
            FiguraCirculo.desenharCirculo(g, (int)x1, (int)y1, (int)x2, (int)y2, "", getEsp(), getCorAtual());
            ...
            //Registra um novo elemento a lista ligada
            listaSave.adicionarRetaLista("CIRCULO", x1/950, y1/600, x2/950, y2/600, getEsp(), getCorAtual());
        }

        //Caso a opção selecionada seja o pincel
        if (tipo == TipoPrimitivo.PINCEL){
            FiguraPinceis.desenharPincel(g, (int)x1, (int)y1, (int)x2, (int)y2, "", getEsp(), getCorAtual());
        }

        //Caso a opção selecionada seja a borracha
        if (tipo == TipoPrimitivo.BORRACHA){
            FiguraPinceis.desenharPincel(g, (int)x1, (int)y1, (int)x2, (int)y2, "", getEsp(), corBorracha);
        }

        if (tipo == TipoPrimitivo.FLOR){
            FiguraFlor.desenharFlor(g, (int)x1, (int)y1, (int)x2, (int)y2, "", getCor1(), getCor2(), getEsp());

            //Salva o circulo que foi desenhado
            String pontoDesenhado = "FLOR: " + "x1(" + x1 + "), y1(" + y1 + "), " + "x2(" + x2 + "), y2(" + y2 + "), " + "esp(" + getEsp() + "), " + "cor1(" + getCorAtual() + ")" + " --> ";
            fileM.escrevaArquivo(pontoDesenhado);

            //Registra um novo elemento a lista ligada
            listaSave.adicionarFlorLista("FLOR", x1/950, y1/600, x2/950, y2/600, getEsp(), getCor1(), getCor2());
        }

        //Caso a opção selecionada seja o refazer
        if (tipo == TipoPrimitivo.REFAZER){
            listaSave.refazerAcao(g);
        }

        // Caso a opcao selecionada seja Salvar, passa a lista pro JSONWriter e escreve
        if (tipo == TipoPrimitivo.SALVARARQ){
            listaSave.repintar(g);
            JSONWriter json = new JSONWriter(listaSave);
            json.escrever();
            listaSave.repintar(g);
        }

        // Caso a opcao selecionada seja Ler, recebe uma nova lista do JSONReader e repinta td
        if (tipo == TipoPrimitivo.LERARQ){
        ...
        // Caso a opcao selecionada seja Limpaar, cria uma nova linkedList vazia e escreve
        if (tipo == TipoPrimitivo.LIMPARARQ){
            JSONWriter json = new JSONWriter(listaSave);
            json.limpar();
        }
    }
