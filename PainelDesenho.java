...
public class PainelDesenho extends JPanel implements MouseListener, MouseMotionListener {
    ...
    public void desenharPrimitivos(Graphics g){
        ...
        if (tipo == TipoPrimitivo.FLOR){
            FiguraFlor.desenharFlor(g, x1, y1, x2, y2, "", getEsp(), getCorAtual());

            //Salva o circulo que foi desenhado
            String pontoDesenhado = "FLOR: " + "x1(" + x1 + "), y1(" + y1 + "), " + "x2(" + x2 + "), y2(" + y2 + "), " + "esp(" + getEsp() + "), " + "cor(" + getCorAtual() + ")" + " --> ";
            fileM.escrevaArquivo(pontoDesenhado);

            //Registra um novo elemento a lista ligada
            listaSave.adicionarRetaLista("FLOR", x1, y1, x2, y2, getEsp(), getCorAtual());
        }
